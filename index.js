...
function indekseGoreFenomen(obje1, i /*kod*/) {
  return i + ". indekste bulunan fenomen: " + obje1[i].profile;
}
...
function fenomenSil(fenomenler, x /*kod*/) {
  const newFenomenler = [];
  for (let i = 0; i < fenomenler.length; i++) {
    if (i !== x) {
      newFenomenler.push(fenomenler[i]);
    }
  }
  return newFenomenler;
...
function fenomenEkle(
  ...
  let yeniFenomenler = (fenomenler[fenomenler.length] = {
    number: sayi,
    profile: profil,
    followers: takipci,
    posts: gonderi,
...
function enFenomenler(fenomenler /*kod*/) {
  let mostPopular = [];
  for (let i = 0; i < fenomenler.length; i++) {
    if (fenomenler[i].followers > 100000000) {
      mostPopular.push(fenomenler[i].profile);
    }
  }
  return mostPopular;
}
